#------------------------------------------------------------------------------
# ECS Module - Production Environment
#------------------------------------------------------------------------------
...
inputs = {
  ...
  # Docker image for production - must be set via TF_VAR_docker_image environment variable
  docker_image   = get_env("TF_VAR_docker_image", "nginx:latest")
  container_name = "gogs-app"
  container_port = 8080
  
  # Task configuration - larger for production
  task_cpu    = 512
  task_memory = 1024
  
  # Higher count for production
  desired_count = 2
